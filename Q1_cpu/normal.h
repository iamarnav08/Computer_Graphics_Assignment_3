...
Vector3f* calculateFaceNormals(OffModel* model) {
    ...
    for(int i = 0; i < model->numberOfPolygons; i++) {
        ...
        // Get three vertices of the polygon (using first three vertices to calculate normal)
...
// Create a buffer to hold exploded triangles
...
// Add the three vertices of this triangle to our buffer, with displacement
for (int j = 0; j < 3; j++) {
int vertIdx = model->polygons[i].v[j];
Vertex v = model->vertices[vertIdx];

// Apply the explosion displacement
v.x += displacement.x;
v.y += displacement.y;
v.z += displacement.z;
